...
def load_tokens(server_name):
    ...
    try:
        ...
        if server_name == "IND":
            url = BASE_URL + "token_ind.json" + cache_buster
        elif server_name in {"BR", "NA", "US", "SAC"}:
            url = BASE_URL + "token_br.json" + cache_buster
        else:
            url = BASE_URL + "token_ag.json" + cache_buster
            
        token = os.environ.get("GITHUB_TOKEN", "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")
...
def root():
    return jsonify({
        ...
        "author": "!Nicchen",
        "endpoint": "/like?uid={UID}&server_name={SERVER_NAME}"
    })
